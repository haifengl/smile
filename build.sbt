...
lazy val root = project.in(file(".")).settings(nonPubishSettings: _*)
  .aggregate(core, data, io, math, netlib, graph, plot, interpolation, nlp, demo, benchmark, scala, shell)
...
lazy val core = project.in(file("core")).settings(commonSettings: _*).dependsOn(data, math, graph, io % "test", netlib % "test")
