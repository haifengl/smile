 ...
 * distributions to be the ratio of the variance between the classes to the
 * variance within the classes, which is, in some sense, a measure of the
 * signal-to-noise ratio for the class labeling. FLD finds a linear combination
 * of features which maximizes the separation after the projection. The
 * resulting combination may be used for dimensionality reduction before later
 * classification.
 * <p>
 ...
 * normally distributed classes or equal class covariances. When the assumptions
 * of LDA are satisfied, FLD is equivalent to LDA.
 * <p>
 * FLD is also closely related to principal component analysis (PCA), which also
 * looks for linear combinations of variables which best explain the data. As a
 * supervised method, FLD explicitly attempts to model the difference between
 * the classes of data. On the other hand, PCA is a unsupervised method and does
 * not take into account any difference in class.
 * <p>
 * One complication in applying FLD (and LDA) to real data occurs when the
 * number of variables/features does not exceed the number of samples. In this
 * case, the covariance estimates do not have full rank, and so cannot be
 * inverted. This is known as small sample size problem.
 * 
 * @see LDA
 ...
 *      <h2>References</h2>
 *      <ol>
 *      <li>Robust and Accurate Cancer Classification with Gene Expression
 *      Profiling http://alumni.cs.ucr.edu/~hli/paper/hli05tumor.pdf.</li>
 *      </ol>
 * 
 * @author Haifeng Li
 */
public class FLD implements Classifier<double[]>, Projection<double[]>, Serializable {
    ...
    /**
     * flag to indicate which numerical calculation to use for training, data set
     * which has dimensions <b>less than or equals to 4</b> would take LOW_DIM model
     * as default.
     */
    ...
    /**
     * 
     * For data set like Iris flower which has only 4 variables, it is probably
     * better that we directly calculate inv(S_t) * S_b which works on well on small
     * dimension data set. For other data with high dimensions, we might use a fast
     * algorithm as described in the reference paper.
     *
     */
    public enum Model {
        /**
        ...
        LOW_DIM,
        /**
         * data set with high dimensions like USPS or Pendigits which has more than 15
         * features
         */
        HIGH_DIM;
    }
    ...
    public static class Trainer extends ClassifierTrainer<double[]> {
        ...
        private int L = -1;
        /**
         * A tolerance to decide if a covariance matrix is singular. The trainer will
         * reject variables whose variance is less than tol<sup>2</sup>.
         */
     ...
     *            the dimensionality of mapped space.
     * @param tol
     *            a tolerance to decide if a covariance matrix is singular; it will
     *            reject variables whose variance is less than tol<sup>2</sup>.
     * @param model
     *            flag to indicate which numerical calculation to use for training
     */
    public FLD(double[][] x, int[] y, int L, double tol, Model... model) {
        ...
        DenseMatrix U = (this.model == Model.LOW_DIM) ? getMappingVectorForLowDim(eigen, B, s)
                : getMappingVectorForHighDim(eigen, ni, n);
     ...
     * 
     * @param eigen
     *            SVD calculator from covariance matrix
     * @param ni
     *            empirical instance count for each class
     * @param n
     *            total training instance count
    ...
    private DenseMatrix getMappingVectorForHighDim(EVD eigen, int[] ni, int n) {
        //
        // calculate the square root of diagonal inverse of eigenvalue matrix from SVD
     ...
     * @param s
     *            eigen value array from SVD of covariance matrix
