 ...
 * <pre>Lundberg, Scott M., and Su-In Lee. “A unified approach to interpreting model predictions.” Advances in Neural Information Processing Systems. 2017 </pre>
 ...
 * <pre>Lundberg, Scott M., Gabriel G. Erion, and Su-In Lee. “Consistent individualized feature attribution for tree ensembles.” arXiv preprint arXiv:1802.03888 (2018) </pre>
 * implementation reference as below:
 * <pre>https://github.com/slundberg/shap/blob/master/shap/explainers/pytree.py</pre>
 * support only {@link RegressionTree} or ensemble based on it like {@link smile.regression.GradientTreeBoost}
 * <p>
 * @author ray
 * @see <pre>https://christophm.github.io/interpretable-ml-book/shap.html</pre>
 *
 */
public class TreeShapImportance{
	...
	public static class TreeShapExplainer{
		...
		/** constructor with tree ensemble and normalization choice (if true, shap value will be normalized) */
		public TreeShapExplainer(RegressionTree[] trees, boolean normalize) {
	        ...
	        List<ConcurrentShapConstruction> concurrencyRunnables = new ArrayList<ConcurrentShapConstruction>(trees.length);
			for(int i = 0;i < trees.length;i++) {
				ConcurrentShapConstruction shapRunnable = new ConcurrentShapConstruction(trees[i], concurrencySentinel, this.normalize);
		...
		/**
		 * return shap value for given data instance, 
		 * note this might take a fairly large memory if the dataset is big.
		 * <p>
		 * This is useful if you want to have deep insight about effect of each feature over prediction 
		 * by using returned shap value for every data instance to draw 
		 ...
		 * @param x data {@link Tuple} 
		 * @param returnAbsolute true if you want the feature importance, 
		 *                       just take absolute shap values for every instance, 
		 *                       average by feature and then sort, default false
		 * @param calcTemps calculation registers, one for each tree if ensemble model
		 * @param concurrent if true then use muti-thread for tree ensemble otherwise run in single thread
         */
		...
		/** 
		 *  return the shap importance for the given model
         *  by taking absolute shap values 
         *  for every instance and average by feature.
         *  <p>
         *  Note it might take a fairly long time if the dataset is big 
         *  and the underlying model is complex (many sub-trees, many nodes, large depth, etc)
         *  
         */
		public double[] shapImportance(StructType schema, DataFrame data) {
	        ...
	        List<ConcurrentShapBatchCalculation> operRunnables = new ArrayList<ConcurrentShapBatchCalculation>(concurrencyLevel);
	        for(int xi = 0;xi < data.size();xi += ConcurrentShapBatchCalculation.operSize) {
	        	...
	        	ConcurrentShapBatchCalculation r = new ConcurrentShapBatchCalculation(this, data, concurrencySentinel, xi, (endIdx > data.size()? data.size() : endIdx)); 
	        	operRunnables.add(r);
	        	exec.submit(r);
	        }
	        ...
	        System.out.println("--- complete calculation for " + data.size() + " data, took " + (System.currentTimeMillis() - startTime)/1000 + " seconds -----");
		...
		private static class ConcurrentShapBatchCalculation implements Runnable{
			...
			ConcurrentShapBatchCalculation(TreeShapExplainer explainer, DataFrame data, 
					                       CountDownLatch concurrencySentinel, 
			...
			public void run() {
				...
				System.out.println("complete shap value calculation for data instances at indexes:[" + startIdx + "," + endIdx + ")");
			}			
		}
		...
		private void treeShap(ShapTree tree, Tuple x, double[] phi, 
				              double condition, int condition_feature,
	        ...
	        treeShapRecursive(
	           ...
	           x, phi, 0, 0, 
	           calcTemp.feature_indexes, calcTemp.zero_fractions, calcTemp.one_fractions, calcTemp.pweights,
	 ...
	 */
	private static void extendPath(List<MutableInteger> feature_indexes, 
	    ...
	    for(int i = startIdx;i > -1;i--){
	    	setDouble(pweights, i + 1, pweights.get(i + 1).getValue() + (one_fraction * pweights.get(i).getValue() * (i + 1.0) / uniquePathPlus1));
	    	setDouble(pweights, i, zero_fraction * pweights.get(i).getValue() * (unique_depth - i + 0.0) / uniquePathPlus1);
	    }
	}	
	 ...
	 */
	private static void unwindPath(List<MutableInteger> feature_indexes, 
                                   List<MutableDouble> zero_fractions, 
                                   List<MutableDouble> one_fractions, 
                                   List<MutableDouble> pweights,
	                               int unique_depth, 
	    ...
	    for(int i = startIdx;i > -1;i--) {
	        if(one_fraction != 0){
	           ...
	           next_one_portion = tmp - pweights.get(i).getValue() * zero_fraction * (unique_depth - i + 0.0) / uniquePathPlus1;
	        }else {
	           setDouble(pweights, i, (pweights.get(i).getValue() * uniquePathPlus1) / (zero_fraction * (unique_depth - i + 0.0)));
	        }
	    }
	 ...
	 */
	private static double unwoundPathSum(List<MutableDouble> zero_fractions, 
                                         List<MutableDouble> one_fractions, 
                                         List<MutableDouble> pweights, 
			                             int unique_depth, 
			                             int path_index) {
	    ...
	    for(int i = startIdx;i > -1;i--) {
	        if(one_fraction != 0) {
	           ...
	           total += tmp;
	           next_one_portion = pweights.get(i).getValue() - tmp * zero_fraction * ((unique_depth - i + 0.0) / uniquePathPlus1);
	        }else{
		...
		private ShapTree(RegressionTree tree, boolean normalize) {
			...
			while(n != null) {
				...
				if(n instanceof InternalNode) {
			       ...
			       System.out.println("error node type for tree structure conversion: " + n.getClass() + ":" + n.toString());
				   throw new RuntimeException("error node type for tree structure conversion: " + n.getClass() + ":" + n.toString());
				}
	...
	private static double subtreeValues(Node n) {
		...
		if(n instanceof RegressionNode) {
		   ...
		   throw new RuntimeException("error node type for node value calculation: " + n.getClass() + ":" + n.toString());
		}
		return v;
	}	
	
	private static int computeExpectations(int[] children_left, int[] children_right, double[] node_sample_weight, double[] values, int i, int depth) {
		if(depth < 0) {
		   depth = 0;
		}
		...
		for(int i = 0;i < uniqueDepthPlus1;i++) {
			if(doubleValue) {
			   if(original.get(i) != null && ((MutableDouble)original.get(i)).getValue() != Double.MIN_VALUE) {
				  setDouble((List<MutableDouble>)original, i + uniqueDepthPlus1, ((MutableDouble)original.get(i)).getValue());					   
			   }			
		    }else {
			   if(original.get(i) != null && ((MutableInteger)original.get(i)).getValue() != Double.MIN_VALUE) {
				  setInteger((List<MutableInteger>)original, i + uniqueDepthPlus1, ((MutableInteger)original.get(i)).getValue());						   
			   }
			}		
		}		
	 ...
	 */
	private static void treeShapRecursive(int[] children_left, 
			                         int[] children_right, 
			                         int[] children_default, 
			                         int[] features, 
			                         Node[] thresholds, 
			                         ...
			                         int node_index, 
			                         int unique_depth, 
			                         List<MutableInteger> parent_feature_indexes,
	    ...
	    if(children_right[node_index] == -1) {
	        ...
	        for(int i = 1;i < loopLength;i++) {
	            ...
	            phi[feature_indexes.get(i).getValue()] += w * (one_fractions.get(i).getValue() - zero_fractions.get(i).getValue()) * values[node_index] * condition_fraction;
	        }
	    } else {// internal node
