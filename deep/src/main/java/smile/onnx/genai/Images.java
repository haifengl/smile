...
/**
 * Loaded images for multimodal GenAI models.
 *
 * @author Haifeng Li
 */
public final class Images implements AutoCloseable {
    /** Native {@code OgaImages*} handle. */
    private MemorySegment handle;

    private Images(MemorySegment handle) {
        this.handle = handle;
    }
     ...
     *
     * @param paths one or more image file paths.
     * @return images owned by the caller.
     */
    public static Images load(String... paths) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            try {
                ...
                MemorySegment images = GenAIRuntime.createHandle(arena,
                        out -> ort_genai_c_h.OgaLoadImages(array, out));
    ...
    MemorySegment handle() {
        GenAIRuntime.requireOpen(handle, "Images");
        return handle;
    }
    ...
    public void close() {
        if (handle != null && !handle.equals(MemorySegment.NULL)) {
            ort_genai_c_h.OgaDestroyImages(handle);
