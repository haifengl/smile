...
public final class Native {
    ...
    private static final class Bindings {
        ...
        static final MethodHandle FLASHINFER_WS_INVALIDATE = smile_torch_h.SYMBOL_LOOKUP
                ...
                .map(s -> LINKER.downcallHandle(s, FunctionDescriptor.ofVoid(ValueLayout.ADDRESS)))
                .orElse(null);
        static final MethodHandle FLASHINFER_WS_INVALIDATE_PREFILL = smile_torch_h.SYMBOL_LOOKUP
    ...
    /** Clears only prefill gather caches on the workspace. */
    public static void flashInferWorkspaceInvalidatePrefillRuntimeCache(MemorySegment handle) {
        if (handle == null || handle.address() == 0
                || Bindings.FLASHINFER_WS_INVALIDATE_PREFILL == null) {
            return;
        }
        try {
            Bindings.FLASHINFER_WS_INVALIDATE_PREFILL.invokeExact(handle);
        } catch (Throwable ignored) {
            // best-effort
        }
    }
