...
class ClassificationTest {
    ...
    fun testDecisionTree() {
        ...
        val iris = Iris()
        val model = cart(iris.formula, iris.data)
        println(model)
    ...
    fun testAdaBoost() {
        ...
        MathEx.setSeed(19650218) // to get repeatable results.
        val iris = Iris()
        val model = adaboost(iris.formula, iris.data, ntrees = 200,
            maxDepth = 20, maxNodes = 4, nodeSize = 1)
    ...
    fun testRandomForest() {
        ...
        MathEx.setSeed(19650218) // to get repeatable results.
        val iris = Iris()
        val model = randomForest(iris.formula, iris.data, ntrees = 100,
            mtry = 3, maxDepth = 20, maxNodes = 100, nodeSize = 5, seeds = seeds)
    ...
    fun testGradientBoosting() {
        ...
        MathEx.setSeed(19650218) // to get repeatable results.
        val iris = Iris()
        val model = gbm(iris.formula, iris.data, ntrees = 100, maxDepth = 20,
            maxNodes = 6, nodeSize = 5, shrinkage = 0.05, subsample = 0.7)
