...
class ClassificationModelSpec extends Specification {
  ...
  "ClassificationModel" should {
    "random.forest" in {
      ...
      val model = Model.classification("random_forest", formula, train, test, params)
    ...
    "svm" in {
      ...
      val model = Model.classification("svm", formula, scaler(train), scaler(test), params)
      println(s"Training metrics: ${model.train}")
      println(s"Validation metrics: ${model.validation}")
      println(s"Test metrics: ${model.test}")
    ...
    "svm with ensemble" in {
      ...
      params.setProperty("smile.svm.C", "100")
      params.setProperty("smile.svm.type", "ovo")
      val model = Model.classification("svm", formula, scaler(train), scaler(test), params, 5, 3, true)
      println(s"Training metrics: ${model.train}")
      println(s"Validation metrics: ${model.validation}")
      println(s"Test metrics: ${model.test}")
    ...
    "mlp" in {
      ...
      val model = Model.classification("mlp", formula, scaler(train), scaler(test), params)
      println(s"Training metrics: ${model.train}")
      println(s"Validation metrics: ${model.validation}")
      println(s"Test metrics: ${model.test}")
