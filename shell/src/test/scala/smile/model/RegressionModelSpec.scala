...
class RegressionModelSpec extends Specification {
  ...
  "RegressionModel" should {
    "random.forest" in {
      ...
      val model = Model.regression("random_forest", formula, train, test, params)
    ...
    "svm" in {
      ...
      val model = Model.regression("svm", formula, train, test, params)
      println(s"Training metrics: ${model.train}")
      println(s"Validation metrics: ${model.validation}")
      println(s"Test metrics: ${model.test}")
    ...
    "svm with ensemble" in {
      ...
      params.setProperty("smile.svm.C", "5")
      params.setProperty("smile.svm.epsilon", "0.5")
      val model = Model.regression("svm", formula, train, test, params, 5, 3, true)
      println(s"Training metrics: ${model.train}")
      println(s"Validation metrics: ${model.validation}")
      println(s"Test metrics: ${model.test}")
    ...
    "mlp" in {
      ...
      val model = Model.regression("mlp", formula, scaler(train), scaler(test), params)
      println(s"Training metrics: ${model.train}")
      println(s"Validation metrics: ${model.validation}")
      println(s"Test metrics: ${model.test}")
