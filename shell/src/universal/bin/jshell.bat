...
if defined BUNDLED_JVM (
  set "_JAVACMD=%BUNDLED_JVM%\bin\jshell.exe"
) else (
  if "%JAVACMD%" neq "" (
    set "_JAVACMD=%JAVACMD%"
  ) else (
    if "%JAVA_HOME%" neq "" (
      if exist "%JAVA_HOME%\bin\jshell.exe" set "_JAVACMD=%JAVA_HOME%\bin\jshell.exe"
    )
  )
)

if "%_JAVACMD%"=="" set _JAVACMD=jshell

rem Detect if this java is ok to use.
for /F %%j in ('"%_JAVACMD%" -version  2^>^&1') do (
  if %%~j==jshell set JAVAINSTALLED=1
